...
def use_process(func):
    ...
    async def process_submitter(
        ...
        future = process_pool.submit(key_wrapper, key, args, kwargs)
        future_list.append(future)
        
        if progress is not None:
            progress.update(func.__name__, advance=1)
            
    process_submitter.__inner_func_name__ = func.__name__
        
    return process_submitter
...
def run_processes(
    ...
    processes: Union[ProcessPoolExecutor,int,None], 
...
) -> Tuple[List[Future],ProcessPoolExecutor]:
    need_close = False
    if isinstance(processes, int) or processes is None:
        process_pool = ProcessPoolExecutor(processes)
    else:
    ...
    if backend == 'trio':
        if not HAS_TRIO_BACKEND:
            raise ImportError('trio backend is not available')
        trio.run(coroutine, func, func_inps, process_pool, future_list, progress)
    elif backend == 'curio':
        if not HAS_CURIO_BACKEND:
            raise ImportError('curio backend is not available')
        curio.run(coroutine, func, func_inps, process_pool, future_list, progress)
    elif backend == 'asyncio':
        asyncio.run(coroutine(func, func_inps, process_pool, future_list, progress))
    if need_close:
        progress.stop()
    return future_list,process_pool
